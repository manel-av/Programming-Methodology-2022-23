...
void Tauler::eliminaCaramel(const Posicio& posIn, const Posicio& posFin, const Direccio& direccio)
{
	...
	case VERTICAL:
		...
		break;
	default:
		m_tauler[posIn.getFila()][posIn.getColumna()].setColor(NO_COLOR);
		m_tauler[posIn.getFila()][posIn.getColumna()].setTipus(NO_TIPUS);
		break;
	}
}
...
bool Tauler::cercaPosVertical(Posicio& posIn, Posicio& posFin, int& n, Candy& c)
{
	...
	while (j < MAX_COLUMNES && !trobat)
	{
		...
		{
			if (m_tauler[i][j].compatibles(m_tauler[i - 1][j]) && m_tauler[i - 1][j].getColor() != NO_COLOR)
			{
			...
			{
				if (n >= 3) // Si hay tres caramelos o más alineados, se calcula la posición final
				{
					posFin.set(i - 1, j);
					trobat = true;
				}
			...
			if (n >= 3 && i == MAX_FILES - 1) // Comprueba si hay una alineación en la última columna
			{
				posFin.set(i, j);
				trobat = true;
			}
	...
	if (trobat)
	{
		...
		c.setTipus(NORMAL);
		n = posFin.getFila() - posIn.getFila() + 1;
		return true;
	}
	else {
		posIn.set(-1, -1);
...
bool Tauler::cercaPosHoritzontal(Posicio& posIn, Posicio& posFin, int& n, Candy& c)
{
	...
	while (i < MAX_FILES && !trobat)
	{
		...
		while (j < MAX_COLUMNES && !trobat)
		{
			if (m_tauler[i][j].compatibles(m_tauler[i][j - 1]) && m_tauler[i][j - 1].getColor() != NO_COLOR)
			{
				if (n == 1) // Calcula la posición inicial
				{
					posIn.set(i, j - 1);
			...
			if (n >= 3 && j == MAX_COLUMNES - 1) // Comprueba si hay una alineación en la última columna
			{
				posFin.set(i, j);
				trobat = true;
			}
	...
	if (trobat)
	{
		c.setColor(m_tauler[posIn.getFila()][posIn.getColumna()].getColor());
		c.setTipus(NORMAL);
		n = posFin.getColumna() - posIn.getColumna() + 1;
		return true;
	}
	else {
		posIn.set(-1, -1);
...
void Tauler::dibuixaTauler()
{
	for (int i = 0; i < MAX_FILES; i++)
	{
		for (int j = 0; j < MAX_COLUMNES; j++)
		{
			m_tauler[i][j].dibuixaCaramel(j, i);
		}
	}
}